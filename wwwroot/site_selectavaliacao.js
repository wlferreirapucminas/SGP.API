let dropdownavaliacao = $('#IdAvaliacao');

dropdownavaliacao.empty();

dropdownavaliacao.append('<option selected="true" disabled>Escolha a avaliação</option>');
...
$.getJSON(urlavaliacao, function (dataavaliacao) {
  $.each(dataavaliacao, function (keyavaliacao, entryavaliacao) {
    dropdownavaliacao.append($('<option></option>').attr('value', entryavaliacao.id).text(entryavaliacao.descricao));
  })
});